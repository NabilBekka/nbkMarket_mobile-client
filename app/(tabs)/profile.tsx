...
function LoginSection({ onSwitchRegister, onSwitchForgot, onNeedsVerify, onGoogleRegister }: {
  ...
  return (
    <View>
      ...
      <TextInput style={[s.input, touchedEmail && !emailValid && email.length > 0 && s.inputError]} placeholder={t.login.emailPlaceholder} placeholderTextColor="#bbb" value={email} onChangeText={setEmail} onBlur={() => setTouchedEmail(true)} keyboardType="email-address" autoCapitalize="none" />
      {touchedEmail && !emailValid && email.length > 0 && <Text style={s.errorText}>{t.login.emailError}</Text>}
      <Text style={[s.label, { marginTop: 14 }]}>{t.login.password}</Text>
      <View><TextInput style={[s.input, { paddingRight: 44 }]} placeholder={t.login.passwordPlaceholder} placeholderTextColor="#bbb" value={password} onChangeText={v => { setPassword(v); setTouchedPw(true); }} secureTextEntry={!showPw} />
        <TouchableOpacity style={s.eyeBtn} onPress={() => setShowPw(!showPw)}><Text style={{ fontSize: 16 }}>{showPw ? "🙈" : "👁️"}</Text></TouchableOpacity></View>
...
function ForgotCodeSection({ email, onSuccess, onBack }: { email: string; onSuccess: () => void; onBack: () => void }) {
  ...
  return (<View><Text style={s.title}>{t.forgot.title}</Text><Text style={s.subtitle}>{t.forgot.codeSent}</Text>
    {success ? <View style={s.successBox}><Text style={s.successText}>✓ {t.forgot.success}</Text></View> : <>
      {error !== "" && <View style={s.errorBox}><Text style={s.errorBoxText}>{error}</Text></View>}
...
const s = StyleSheet.create({
  ...
  googleBtnText: { fontSize: 14, fontWeight: "500", color: colors.textDark },
  ...
  errorTextMsg: { fontSize: 13, color: "#E53935", marginTop: 8, fontWeight: "500" },
  okText: { fontSize: 11, color: "#4CAF50", marginTop: 4, fontWeight: "500" },
  checkText: { fontSize: 11, color: colors.textGray, marginTop: 4 },
  ...
  successBox: { backgroundColor: "#E8F5E9", borderRadius: 10, padding: 14, marginBottom: 16 },
  ...
  submitBtn: { backgroundColor: colors.navyDark, borderRadius: 10, padding: 13, alignItems: "center" },
  ...
  settingsBtn: { marginTop: 24, borderWidth: 1, borderColor: "#ddd", borderRadius: 10, paddingVertical: 12, paddingHorizontal: 28, backgroundColor: "#fff" },
  ...
  editText: { fontSize: 12, fontWeight: "500", color: colors.accent },
  cancelText: { fontSize: 12, fontWeight: "500", color: colors.textGray },
  dangerCard: { backgroundColor: "#fff", borderRadius: 12, padding: 16, borderWidth: 1, borderColor: "#FFCDD2", marginBottom: 16 },
  dangerTitle: { fontSize: 17, fontWeight: "600", color: "#E53935", marginBottom: 6 },
  ...
  modalText: { fontSize: 14, color: colors.textGray, lineHeight: 22, marginBottom: 24 },
  modalActions: { flexDirection: "row", gap: 10 },
  modalCancel: { flex: 1, padding: 12, borderWidth: 1, borderColor: "#ddd", borderRadius: 10, alignItems: "center" },
  ...
  googleInfoName: { fontSize: 14, fontWeight: "600", color: colors.textDark },
  googleInfoEmail: { fontSize: 12, color: colors.textGray },
});
